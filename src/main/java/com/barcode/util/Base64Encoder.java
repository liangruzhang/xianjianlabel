...
/**
 * A utility encoder class for data encoding and decoding using the
 * Base64 algorithm.
 */
public class Base64Encoder
{
    ...
    private final static int DEFAULT_MAX_LINE_LENGTH = 76;
    ...
    public Base64Encoder(int maxLineLength) {
        if (maxLineLength == -1 || maxLineLength > 0) {
            this.maxLineLength = maxLineLength;
        } else {
            throw new IllegalArgumentException(
                "Max line length cannot be " + maxLineLength);
        }
    }
    ...
    public static void main( String[] args )
    {
        ...
        byte[] b = encoder.encodeString(s).getBytes();
        byte[] c = encoder.decode( b, 0, b.length );
        ...
        try
        {
            ...
            Base64Encoder.InputStream b64is = new Base64Encoder.InputStream( fis, DECODE );
            int ib = 0;
...
/* ********  E N C O D I N G   M E T H O D S  ******** */
    ...
    public String encodeObject( java.io.Serializable serializableObject )
    {
        ...
        try
        {
            ...
            b64os = new Base64Encoder.OutputStream( baos, Base64Encoder.ENCODE );
    ...
    public String encodeBytes( byte[] source, int off, int len )
    {
        ...
        byte[] outBuff = new byte[   ( len43 )                      // Main 4:3
            ...
            + ( (maxLineLength > 0) ? len43 / maxLineLength : 0) ]; // New lines
        int d = 0;
        int e = 0;
        int len2 = len - 2;
        int lineLength = 0;
        ...
        public int read() throws java.io.IOException
        {
            ...
            if( position < 0 )
            {
                if( encode )
                {
                    ...
                    if( numSigBytes > 0 )
                    {
                        encoder.encode3to4( b3, 0, numSigBytes, buffer, 0 );
                        position = 0;
                    }   // end if: got data
                }   // end if: encoding
                ...
                else
                {
                    ...
                    if( i == 4 )
                    {
                        numSigBytes = encoder.decode4to3( b4, 0, buffer, 0 );
                        position = 0;
    ...
    public static class OutputStream extends java.io.FilterOutputStream
    {
        ...
        public void write(int theByte) throws java.io.IOException
        {
            ...
            if( position >= bufferLength )
            {
                if( encode )
                {
                    out.write(encoder.encode3to4( buffer, bufferLength ) );

                    lineLength += 4;
                    if( (encoder.maxLineLength > 0) &&
                        (lineLength >= encoder.maxLineLength) )
                    {
                        out.write( NEW_LINE );
                        lineLength = 0;
                    }   // end if: end o fline
                    ...
                    out.write( encoder.decode4to3( buffer ) );
        ...
        public void flush() throws java.io.IOException
        {
            if( position > 0 )
            {
                if( encode )
                {
                    out.write( encoder.encode3to4( buffer, position ) );
                }   // end if: encoding
                else
                {
